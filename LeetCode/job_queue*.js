...
// While working on our application servers, we realized that sometimes we have a long list of asynchronous network requests that need to be
// resolved in a specific, sequential order.
...
// whenever job ends up getting executed. (But, note that addJob itself should not trigger execution of any jobs.) If job throws an error,
// then the promise returned by addJob should be rejected.
...
// When processAllJobs is called, the queue should process jobs (by invoking them) one-at-a-time in FIFO order until there are none left,
// then resolve with the number of jobs successfully processed (i.e., that did not reject or throw an error).
