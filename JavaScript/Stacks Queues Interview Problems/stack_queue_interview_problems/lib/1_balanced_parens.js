...
// make a hash and have a key for for parens, curlys, and brackets
...
function balancedParens(str) {
  ...
  for (let i=0; i<chars.length; i++) {
    ...
    if (pairs[char]) { //if it is an opening item, add it to the stack 
      stack.push(char);
    } else if (char === ')' || char === '}' || char === ']') { //else if it is a closing item, remove from the stack
      if (pairs[stack.pop()] !== char) return false //if it isn't a pair, there is an error and return false
    }
  }
